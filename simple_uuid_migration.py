#!/usr/bin/env python3
"""
Simple UUID migration script for existing database.
This script will convert integer user IDs to UUIDs step by step.
"""
...
def create_backup():
    ...
    print("⚠️  IMPORTANT: Creating database backup...")
    print("Please run this command to backup your database:")
    print("mysqldump -u root fastapi_db > backup_$(date +%Y%m%d_%H%M%S).sql")
    print()
    
    response = input("Have you created a backup? (y/n): ").lower().strip()
    if response not in ['y', 'yes']:
        print("Please create a backup first and run this script again.")
        sys.exit(1)
...
def main():
    ...
    print("Simple UUID Migration Script")
    print("=" * 50)
    print()
    ...
    try:
        ...
        # Step 5: Migrate foreign key tables
        print("\n" + "="*50)
