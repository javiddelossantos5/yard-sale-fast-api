#!/bin/bash
# Script to add the 'miles' column to the items table (PRODUCTION)
# Uses root password: supersecretpassword (from docker-compose)
# Database: yardsale
# Container: yard-sale-db

echo "🚀 Adding 'miles' column to items table (PRODUCTION)..."
...
echo "📋 Attempting to add 'miles' column..."
docker exec -i yard-sale-db mysql -uroot -psupersecretpassword yardsale <<'EOF' 2>/dev/null
ALTER TABLE items ADD COLUMN miles INT NULL COMMENT 'Mileage for automotive items (optional)' AFTER quantity;
EOF

# Check if the command succeeded or if column already exists
if [ $? -eq 0 ]; then
    echo "✅ Column 'miles' added successfully"
    echo ""
    echo "💡 Verifying column was added..."
    docker exec -i yard-sale-db mysql -uroot -psupersecretpassword yardsale -e "DESCRIBE items;" | grep -E "(Field|miles)" || echo "   (Check manually if needed)"
    echo ""
